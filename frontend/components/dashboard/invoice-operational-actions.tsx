...
import { ClipboardPlus, CreditCard, Loader2, ReceiptText } from "lucide-react";
...
function InteractionSheet({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const fechaGestion = String(form.get("fecha_gestion") ?? "");
    ...
    try {
      await onCreate({
        ...
        resultado,
        motivo_no_pago: motivo === "none" ? null : motivo,
        ...
        <form id="interaction-form" onSubmit={handleSubmit} className="overflow-auto px-4">
          <FieldGroup>
            <Field>
              ...
              <Input id="fecha_gestion" name="fecha_gestion" type="date" defaultValue={fechaCorte} required />
            </Field>
            <Field>
...
function PromiseSheet({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setPending(true);
    try {
      await onCreate({ gestion_id: gestionId, fecha_compromiso: fechaCompromiso, recalculate: true });
      event.currentTarget.reset();
      setOpen(false);
    } catch (err) {
